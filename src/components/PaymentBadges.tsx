...
function VisaIcon() {
  return (
    <svg viewBox="0 0 48 32" aria-hidden className="h-auto w-12">
...
export function PaymentBadges({
  ...
  return (
    <div className={className}>
      ...
      <ul
        className={`mt-3 flex flex-nowrap items-center gap-4 ${centered ? 'justify-center' : ''}`}
        ...
        {paymentBadges.map(({ id, label, icon: Icon }) => (
          <li key={id}>
            <span
              className={
                isFooter
                  ? 'inline-flex w-12 items-center justify-center rounded-full bg-white px-2 py-2 shadow-sm'
                  : 'inline-flex w-12 items-center justify-center rounded-md border border-grey-light bg-white px-2 py-1.5 shadow-sm'
              }
              title={label}
              aria-label={label}
            >
